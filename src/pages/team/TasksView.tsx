...
const TasksView = () => {
  ...
  return (
    <Card className="card-elevated animate-fade-in">
      <CardHeader><CardTitle className="flex items-center gap-2"><ClipboardList className="w-5 h-5 text-primary" />المهام</CardTitle></CardHeader>
      <CardContent>
        {tasks.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground"><ClipboardList className="w-12 h-12 mx-auto mb-4 opacity-30" /><p>لا توجد مهام مسجلة</p></div>
        ) : (
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow className="table-header">
                  <TableHead>المهمة</TableHead><TableHead>الحالة</TableHead><TableHead>نسبة الإنجاز</TableHead><TableHead>نسبة التقدم الأسبوعي</TableHead><TableHead>تاريخ آخر تحديث</TableHead><TableHead>آخر تحديث</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {tasks.map((task) => (
                  <TableRow key={task.id} className="animate-fade-in">
                    ...
                    <TableCell className="max-w-xs"><p className="text-sm text-muted-foreground whitespace-pre-wrap">{task.latestUpdate || '-'}</p></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
